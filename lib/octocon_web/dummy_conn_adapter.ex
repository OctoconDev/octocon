defmodule OctoconWeb.DummyConnAdapter do
  ...
  def send_resp(%{method: "HEAD"} = state, _status, _headers, _body) do
    {:ok, "", state}
  end

  def send_resp(state, _status, _headers, body) do
    {:ok, IO.iodata_to_binary(body), state}
  end

  def send_file(%{method: "HEAD"} = state, _status, _headers, _path, _offset, _length) do
    {:ok, "", state}
  end

  def send_file(state, _status, _headers, _path, _offset, _length) do
    {:ok, "", state}
  end
  ...
  def upgrade(_state, :not_supported, _opts) do
    {:error, :not_supported}
  end

  def upgrade(state, _protocol, _opts) do
    {:ok, state}
  end
  ...
  defp body_or_params(params, query, headers, _method) when is_map(params) do
    ...
    {params, {"--dummy_conn--", body_params}, {query, query_params}, headers}
  end
