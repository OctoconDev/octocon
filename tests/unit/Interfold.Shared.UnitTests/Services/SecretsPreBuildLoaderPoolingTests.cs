...
// pool, or MaxPoolSize too small with no gate) or "too many clients already" (loader pool
// unbounded) integration-suite failures.
public sealed class SecretsPreBuildLoaderPoolingTests
{
    ...
    [Test]
    public async Task WithDedicatedPoolIdentity_StampsLoaderTimeout()
    {
        const string input =
            "Host=localhost;Port=5432;Username=app;Password=pw;Database=octocon;Timeout=15";

        var output = SecretsPreBuildLoader.WithDedicatedPoolIdentity(input);

        var parsed = new NpgsqlConnectionStringBuilder(output);
        await Assert.That(parsed.Timeout).IsEqualTo(SecretsPreBuildLoader.LoaderTimeoutSeconds);
    }
    ...
    public async Task WithDedicatedPoolIdentity_PreservesAllOtherKeywords()
    {
        const string input =
            ...
            "SSL Mode=Require;Command Timeout=60";
